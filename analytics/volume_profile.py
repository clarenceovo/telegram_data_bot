...
of traded volume by price, not tick-level trade reconstruction. KDE peak widths
are half-prominence zones, never confidence intervals. ``stable`` only tests
peak sensitivity to bandwidths 0.8 and 1.2 times the configured value. The caller
must exclude incomplete sessions; no clock or future data is consulted here.
The KDE grid extends beyond observed prices to detect modes at observed extrema;
reported zones are clipped to the observed positive-volume price range.
"""
...
def analyze_volume_profile(bars: pd.DataFrame, *, lookback_sessions=60,
                           bins=48, bandwidth=0.2, prominence=0.1) -> VolumeProfile:
    ...
    Inputs must already be unique, ordered, timezone-aware completed bars with
    positive close and nonnegative, incremental volume. Latest close includes
    zero-volume bars, but they contribute no profile mass.
    """
